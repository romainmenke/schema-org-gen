...
StructuredValue// EligibleQuantity see : /eligibleQuantity
...
// EligibleTransactionVolume see : /eligibleTransactionVolume
// The transaction volume, in a monetary unit, for which the offer or price specification is valid, e.g. for indicating a minimal purchasing volume, to express free shipping above a certain order volume, or to limit the acceptance of credit cards to purchases to a certain minimal amount. 
EligibleTransactionVolume interface{} `json:"eligibleTransactionVolume"` // types : Demand Offer PriceSpecification
...
PriceSpecification interface{} `json:"priceSpecification"` // types : Demand Offer TradeAction
...
TotalPrice interface{} `json:"totalPrice"` // types : Reservation Ticket
