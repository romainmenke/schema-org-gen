...
class LodgingReservation implements \JsonSerializable {
	 ...
	 * @var string | string[]
	 */
	public $lodging_unit_description;
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->checkin_time );
		if ( ! empty( $serialized ) ) {
			$out['checkinTime'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->checkout_time );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->lodging_unit_description );
		if ( ! empty( $serialized ) ) {
			$out['lodgingUnitDescription'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->lodging_unit_type );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->num_adults );
		if ( ! empty( $serialized ) ) {
			$out['numAdults'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->num_children );
		if ( ! empty( $serialized ) ) {
