...
class VideoGameSeries implements \JsonSerializable {
	 ...
	 *
	 * @var string | string[] | \Place | \Place[] | \PostalAddress | \PostalAddress[]
	 */
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->character_attribute );
		if ( ! empty( $serialized ) ) {
			$out['characterAttribute'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->cheat_code );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->contains_season );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->episode );
		if ( ! empty( $serialized ) ) {
			$out['episode'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->episodes );
		if ( ! empty( $serialized ) ) {
			$out['episodes'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->game_item );
		if ( ! empty( $serialized ) ) {
			$out['gameItem'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->game_location );
		if ( ! empty( $serialized ) ) {
			$out['gameLocation'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->game_platform );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->issn );
		if ( ! empty( $serialized ) ) {
			$out['issn'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->number_of_episodes );
		if ( ! empty( $serialized ) ) {
			$out['numberOfEpisodes'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->number_of_players );
		if ( ! empty( $serialized ) ) {
			$out['numberOfPlayers'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->number_of_seasons );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->play_mode );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->quest );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->season );
		if ( ! empty( $serialized ) ) {
			$out['season'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->seasons );
		if ( ! empty( $serialized ) ) {
			$out['seasons'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->trailer );
		if ( ! empty( $serialized ) ) {
