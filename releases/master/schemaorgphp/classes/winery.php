...
// Winery see : https://schema.org/Winery
class Winery implements \JsonSerializable {

	public static $context = 'http://schema.org';
	public static $type    = 'Winery';

	/**
	 * With properties from FoodEstablishment see : https://schema.org/FoodEstablishment
	 */
	 ...
	 *
	 * @var string | string[] | boolean | boolean[]
	 */
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->accepts_reservations );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->has_menu );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->menu );
		if ( ! empty( $serialized ) ) {
			$out['menu'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->serves_cuisine );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->star_rating );
		if ( ! empty( $serialized ) ) {
