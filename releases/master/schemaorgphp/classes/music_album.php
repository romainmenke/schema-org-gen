...
class MusicAlbum implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->album_production_type );
		if ( ! empty( $serialized ) ) {
			$out['albumProductionType'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->album_release );
		if ( ! empty( $serialized ) ) {
			$out['albumRelease'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->album_release_type );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->by_artist );
		if ( ! empty( $serialized ) ) {
