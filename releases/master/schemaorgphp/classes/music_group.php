...
class MusicGroup implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->album );
		if ( ! empty( $serialized ) ) {
			$out['album'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->albums );
		if ( ! empty( $serialized ) ) {
			$out['albums'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->music_group_member );
		if ( ! empty( $serialized ) ) {
			$out['musicGroupMember'] = $serialized;
		}
