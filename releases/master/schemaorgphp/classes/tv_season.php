...
class TVSeason implements \JsonSerializable {
	...
	/**
	 * With properties from CreativeWorkSeason see : https://schema.org/CreativeWorkSeason
	 */
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->country_of_origin );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->part_of_series );
		if ( ! empty( $serialized ) ) {
			$out['partOfSeries'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->part_of_tv_series );
		if ( ! empty( $serialized ) ) {
			$out['partOfTVSeries'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->season_number );
		if ( ! empty( $serialized ) ) {
