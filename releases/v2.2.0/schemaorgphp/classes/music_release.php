...
class MusicRelease implements \JsonSerializable {

	public static $context = 'http://schema.org';
	public static $type    = 'MusicRelease';

	/**
	 * With properties from MusicPlaylist see : https://schema.org/MusicPlaylist
	 */
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->catalog_number );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->credited_to );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->music_release_format );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->num_tracks );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->record_label );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->release_of );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->track );
		if ( ! empty( $serialized ) ) {
			$out['track'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->tracks );
		if ( ! empty( $serialized ) ) {
			$out['tracks'] = $serialized;
		}
