...
// Muscle see : https://schema.org/Muscle
class Muscle implements \JsonSerializable {

	public static $context = 'http://schema.org';
	public static $type    = 'Muscle';
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->action );
		if ( ! empty( $serialized ) ) {
			$out['action'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->antagonist );
		if ( ! empty( $serialized ) ) {
			$out['antagonist'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->blood_supply );
		if ( ! empty( $serialized ) ) {
			$out['bloodSupply'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->insertion );
		if ( ! empty( $serialized ) ) {
			$out['insertion'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->muscle_action );
		if ( ! empty( $serialized ) ) {
			$out['muscleAction'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->nerve );
		if ( ! empty( $serialized ) ) {
			$out['nerve'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->origin );
		if ( ! empty( $serialized ) ) {
			$out['origin'] = $serialized;
		}
