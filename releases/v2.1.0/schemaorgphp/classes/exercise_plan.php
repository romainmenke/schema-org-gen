...
class ExercisePlan implements \JsonSerializable {

	public static $context = 'http://schema.org';
	public static $type    = 'ExercisePlan';
	...
	/**
	 * With properties from PhysicalActivity see : https://schema.org/PhysicalActivity
	 */
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->activity_duration );
		if ( ! empty( $serialized ) ) {
			$out['activityDuration'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->activity_frequency );
		if ( ! empty( $serialized ) ) {
			$out['activityFrequency'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->additional_variable );
		if ( ! empty( $serialized ) ) {
			$out['additionalVariable'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->exercise_type );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->intensity );
		if ( ! empty( $serialized ) ) {
			$out['intensity'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->repetitions );
		if ( ! empty( $serialized ) ) {
			$out['repetitions'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->rest_periods );
		if ( ! empty( $serialized ) ) {
			$out['restPeriods'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->workload );
		if ( ! empty( $serialized ) ) {
			$out['workload'] = $serialized;
		}
