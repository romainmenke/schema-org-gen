...
class APIReference implements \JsonSerializable {

	public static $context = 'http://schema.org';
	public static $type    = 'APIReference';

	/**
	 * With properties from TechArticle see : https://schema.org/TechArticle
	 */

	/**
	 * With properties from Article see : https://schema.org/Article
	 */
	 ...
	 * @var string | string[]
	 */
	public $executable_library_name;
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->article_body );
		if ( ! empty( $serialized ) ) {
			$out['articleBody'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->article_section );
		if ( ! empty( $serialized ) ) {
			$out['articleSection'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->assembly );
		if ( ! empty( $serialized ) ) {
			$out['assembly'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->assembly_version );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->dependencies );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->executable_library_name );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->page_end );
		if ( ! empty( $serialized ) ) {
			$out['pageEnd'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->page_start );
		if ( ! empty( $serialized ) ) {
			$out['pageStart'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->pagination );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->proficiency_level );
		if ( ! empty( $serialized ) ) {
			$out['proficiencyLevel'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->programming_model );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->report_number );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->target_platform );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->word_count );
		if ( ! empty( $serialized ) ) {
