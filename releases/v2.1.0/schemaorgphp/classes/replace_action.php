...
class ReplaceAction implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->collection );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->replacee );
		if ( ! empty( $serialized ) ) {
			$out['replacee'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->replacer );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->target_collection );
		if ( ! empty( $serialized ) ) {
