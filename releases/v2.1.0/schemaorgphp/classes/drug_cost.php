...
class DrugCost implements \JsonSerializable {

	public static $context = 'http://schema.org';
	public static $type    = 'DrugCost';
	 ...
	 *
	 * @var \DrugCostCategory | \DrugCostCategory[]
	 */
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->applicable_location );
		if ( ! empty( $serialized ) ) {
			$out['applicableLocation'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->cost_category );
		if ( ! empty( $serialized ) ) {
			$out['costCategory'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->cost_currency );
		if ( ! empty( $serialized ) ) {
			$out['costCurrency'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->cost_origin );
		if ( ! empty( $serialized ) ) {
			$out['costOrigin'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->cost_per_unit );
		if ( ! empty( $serialized ) ) {
			$out['costPerUnit'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->drug_unit );
		if ( ! empty( $serialized ) ) {
			$out['drugUnit'] = $serialized;
		}
