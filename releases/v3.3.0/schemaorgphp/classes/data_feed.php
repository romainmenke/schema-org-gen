...
class DataFeed implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->catalog );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->data_feed_element );
		if ( ! empty( $serialized ) ) {
			$out['dataFeedElement'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->dataset_time_interval );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->distribution );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->included_data_catalog );
		if ( ! empty( $serialized ) ) {
			$out['includedDataCatalog'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->included_in_data_catalog );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->spatial );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->temporal );
		if ( ! empty( $serialized ) ) {
