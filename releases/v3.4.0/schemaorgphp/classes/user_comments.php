...
class UserComments implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->comment_text );
		if ( ! empty( $serialized ) ) {
			$out['commentText'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->comment_time );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->discusses );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->reply_to_url );
		if ( ! empty( $serialized ) ) {
