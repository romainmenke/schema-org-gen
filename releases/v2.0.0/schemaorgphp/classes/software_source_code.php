...
class SoftwareSourceCode implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->code_repository );
		if ( ! empty( $serialized ) ) {
			$out['codeRepository'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->code_sample_type );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->programming_language );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->runtime );
		if ( ! empty( $serialized ) ) {
			$out['runtime'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->runtime_platform );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->sample_type );
		if ( ! empty( $serialized ) ) {
			$out['sampleType'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->target_product );
		if ( ! empty( $serialized ) ) {
