...
// ImagingTest see : https://schema.org/ImagingTest
class ImagingTest implements \JsonSerializable {

	public static $context = 'http://schema.org';
	public static $type    = 'ImagingTest';
	 ...
	 *
	 * @var \MedicalImagingTechnique | \MedicalImagingTechnique[]
	 */
	 ...
	 */
	public $used_to_diagnose;

	/**
	 * Device used to perform the test.
	 * see : https://schema.org/usesDevice
	 *
	 * @var \MedicalDevice | \MedicalDevice[]
	 */
	public $uses_device;
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->affected_by );
		if ( ! empty( $serialized ) ) {
			$out['affectedBy'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->imaging_technique );
		if ( ! empty( $serialized ) ) {
			$out['imagingTechnique'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->normal_range );
		if ( ! empty( $serialized ) ) {
			$out['normalRange'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->sign_detected );
		if ( ! empty( $serialized ) ) {
			$out['signDetected'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->used_to_diagnose );
		if ( ! empty( $serialized ) ) {
			$out['usedToDiagnose'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->uses_device );
		if ( ! empty( $serialized ) ) {
			$out['usesDevice'] = $serialized;
		}
