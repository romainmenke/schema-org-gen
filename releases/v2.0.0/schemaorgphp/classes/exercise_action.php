...
class ExerciseAction implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->diet );
		if ( ! empty( $serialized ) ) {
			$out['diet'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->exercise_plan );
		if ( ! empty( $serialized ) ) {
			$out['exercisePlan'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->exercise_related_diet );
		if ( ! empty( $serialized ) ) {
