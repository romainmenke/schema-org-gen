...
// MedicalTrial see : https://schema.org/MedicalTrial
class MedicalTrial implements \JsonSerializable {

	public static $context = 'http://schema.org';
	public static $type    = 'MedicalTrial';
	 ...
	 * see : https://schema.org/population
	 *
	 * @var string | string[]
	 */
	public $population;
	 ...
	 *
	 * @var \MedicalStudyStatus | \MedicalStudyStatus[]
	 */
	public $status;
	 ...
	 *
	 * @var \MedicalTrialDesign | \MedicalTrialDesign[]
	 */
	public $trial_design;
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->outcome );
		if ( ! empty( $serialized ) ) {
			$out['outcome'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->phase );
		if ( ! empty( $serialized ) ) {
			$out['phase'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->population );
		if ( ! empty( $serialized ) ) {
			$out['population'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->status );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->study_location );
		if ( ! empty( $serialized ) ) {
			$out['studyLocation'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->study_subject );
		if ( ! empty( $serialized ) ) {
			$out['studySubject'] = $serialized;
		}

		$serialized = \SchemaOrg\json_serialize( $this->trial_design );
		if ( ! empty( $serialized ) ) {
			$out['trialDesign'] = $serialized;
		}
