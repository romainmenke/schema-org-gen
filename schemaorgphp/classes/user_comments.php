...
// UserComments see : https://schema.org/UserComments
class UserComments implements \JsonSerializable {

	public static $context = 'http://schema.org';
	public static $type    = 'UserComments';
	...
	public function jsonSerialize() {
		...
		$serialized = so_json_serialize( $this->attendees );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = so_json_serialize( $this->comment_text );
		if ( ! empty( $serialized ) ) {
			$out['commentText'] = $serialized;
		}

		$serialized = so_json_serialize( $this->comment_time );
		if ( ! empty( $serialized ) ) {
			$out['commentTime'] = $serialized;
		}
		...
		$serialized = so_json_serialize( $this->discusses );
		if ( ! empty( $serialized ) ) {
			$out['discusses'] = $serialized;
		}
		...
		$serialized = so_json_serialize( $this->performers );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = so_json_serialize( $this->reply_to_url );
		if ( ! empty( $serialized ) ) {
			$out['replyToUrl'] = $serialized;
		}
		...
		$serialized = so_json_serialize( $this->sub_events );
		if ( ! empty( $serialized ) ) {
