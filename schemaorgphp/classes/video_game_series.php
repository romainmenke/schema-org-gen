...
class VideoGameSeries implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = so_json_serialize( $this->actors );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = so_json_serialize( $this->directors );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = so_json_serialize( $this->episodes );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = so_json_serialize( $this->season );
		if ( ! empty( $serialized ) ) {
			$out['season'] = $serialized;
		}

		$serialized = so_json_serialize( $this->seasons );
		if ( ! empty( $serialized ) ) {
