...
class Book implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->abridged );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->book_edition );
		if ( ! empty( $serialized ) ) {
			$out['bookEdition'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->book_format );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->illustrator );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->isbn );
		if ( ! empty( $serialized ) ) {
			$out['isbn'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->number_of_pages );
		if ( ! empty( $serialized ) ) {
