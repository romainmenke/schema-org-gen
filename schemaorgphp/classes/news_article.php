...
class NewsArticle implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->dateline );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->print_column );
		if ( ! empty( $serialized ) ) {
			$out['printColumn'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->print_edition );
		if ( ! empty( $serialized ) ) {
			$out['printEdition'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->print_page );
		if ( ! empty( $serialized ) ) {
			$out['printPage'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->print_section );
		if ( ! empty( $serialized ) ) {
