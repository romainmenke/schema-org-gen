...
class LiveBlogPosting implements \JsonSerializable {
	...
	/**
	 * With properties from BlogPosting see : https://schema.org/BlogPosting
	 */
	...
	/**
	 * With properties from SocialMediaPosting see : https://schema.org/SocialMediaPosting
	 */
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->coverage_end_time );
		if ( ! empty( $serialized ) ) {
			$out['coverageEndTime'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->coverage_start_time );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->live_blog_update );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->shared_content );
		if ( ! empty( $serialized ) ) {
