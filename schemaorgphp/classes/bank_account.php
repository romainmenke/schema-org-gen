...
class BankAccount implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->account_minimum_inflow );
		if ( ! empty( $serialized ) ) {
			$out['accountMinimumInflow'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->account_overdraft_limit );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->bank_account_type );
		if ( ! empty( $serialized ) ) {
