...
class BroadcastEvent implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->broadcast_of_event );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->is_live_broadcast );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->published_by );
		if ( ! empty( $serialized ) ) {
			$out['publishedBy'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->published_on );
		if ( ! empty( $serialized ) ) {
