...
class VisualArtwork implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->art_edition );
		if ( ! empty( $serialized ) ) {
			$out['artEdition'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->art_medium );
		if ( ! empty( $serialized ) ) {
			$out['artMedium'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->artform );
		if ( ! empty( $serialized ) ) {
			$out['artform'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->artist );
		if ( ! empty( $serialized ) ) {
			$out['artist'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->artwork_surface );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->colorist );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->inker );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->letterer );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->penciler );
		if ( ! empty( $serialized ) ) {
