...
class FlightReservation implements \JsonSerializable {
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->boarding_group );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->passenger_priority_status );
		if ( ! empty( $serialized ) ) {
			$out['passengerPriorityStatus'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->passenger_sequence_number );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->security_screening );
		if ( ! empty( $serialized ) ) {
