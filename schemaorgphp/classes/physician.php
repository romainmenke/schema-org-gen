...
class Physician implements \JsonSerializable {

	public static $context = 'http://schema.org';
	public static $type = 'Physician';
	
	/**
	 * With properties from MedicalOrganization see : https://schema.org/MedicalOrganization
	 */
	 ...
	 * @var boolean | boolean[]
	 */
	public $is_accepting_new_patients;
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->health_plan_network_id );
		if ( ! empty( $serialized ) ) {
			$out['healthPlanNetworkId'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->hospital_affiliation );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->is_accepting_new_patients );
		if ( ! empty( $serialized ) ) {
