...
class Course implements \JsonSerializable {
	 ...
	 * @var string | string[]
	 */
	public $educational_credential_awarded;
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->course_code );
		if ( ! empty( $serialized ) ) {
			$out['courseCode'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->course_prerequisites );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->educational_credential_awarded );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->has_course_instance );
		if ( ! empty( $serialized ) ) {
