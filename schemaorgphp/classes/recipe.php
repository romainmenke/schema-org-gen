...
class Recipe implements \JsonSerializable {
	...
	/**
	 * With properties from HowTo see : https://schema.org/HowTo
	 */
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->cook_time );
		if ( ! empty( $serialized ) ) {
			$out['cookTime'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->cooking_method );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->estimated_cost );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->nutrition );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->recipe_category );
		if ( ! empty( $serialized ) ) {
			$out['recipeCategory'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->recipe_cuisine );
		if ( ! empty( $serialized ) ) {
			$out['recipeCuisine'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->recipe_ingredient );
		if ( ! empty( $serialized ) ) {
			$out['recipeIngredient'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->recipe_instructions );
		if ( ! empty( $serialized ) ) {
			$out['recipeInstructions'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->recipe_yield );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->step );
		if ( ! empty( $serialized ) ) {
			$out['step'] = $serialized;
		}
		...
		$serialized = \SchemaOrg\json_serialize( $this->suitable_for_diet );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->yield );
		if ( ! empty( $serialized ) ) {
