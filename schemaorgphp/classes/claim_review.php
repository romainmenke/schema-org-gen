...
class ClaimReview implements \JsonSerializable {
	...
	/**
	 * With properties from Review see : https://schema.org/Review
	 */
	...
	public function jsonSerialize() {
		...
		$serialized = \SchemaOrg\json_serialize( $this->claim_reviewed );
		if ( ! empty( $serialized ) ) {
		...
		$serialized = \SchemaOrg\json_serialize( $this->review_aspect );
		if ( ! empty( $serialized ) ) {
			$out['reviewAspect'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->review_body );
		if ( ! empty( $serialized ) ) {
			$out['reviewBody'] = $serialized;
		}
		
		$serialized = \SchemaOrg\json_serialize( $this->review_rating );
		if ( ! empty( $serialized ) ) {
