...
// PostOffice see : https://schema.org/PostOffice
type PostOffice struct {
	typeContext

	// With properties from GovernmentOffice see : https://schema.org/GovernmentOffice
	//
...
func (v PostOffice) intoMap(intop *map[string]interface{}) error {
	if intop == nil {
		return nil
	}
...
func (v PostOffice) asMap() (map[string]interface{}, error) {
	...
	data["@type"] = "PostOffice"

	return data, nil
}

func (v PostOffice) MarshalJSON() ([]byte, error) {
	data, err := v.asMap()
	if err != nil {
		return nil, err
	}
