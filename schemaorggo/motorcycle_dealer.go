...
// MotorcycleDealer see : https://schema.org/MotorcycleDealer
type MotorcycleDealer struct {
	typeContext

	// With properties from AutomotiveBusiness see : https://schema.org/AutomotiveBusiness
	//
...
func (v MotorcycleDealer) intoMap(intop *map[string]interface{}) error {
	if intop == nil {
		return nil
	}
...
func (v MotorcycleDealer) asMap() (map[string]interface{}, error) {
	...
	data["@type"] = "MotorcycleDealer"

	return data, nil
}

func (v MotorcycleDealer) MarshalJSON() ([]byte, error) {
	data, err := v.asMap()
	if err != nil {
		return nil, err
	}
