...
// Sculpture see : https://schema.org/Sculpture
type Sculpture struct {
	...
	AccessMode string `json:"accessMode,omitempty"` // types : Text

	// AccessModeSufficient see : https://schema.org/accessModeSufficient
	// A list of single or combined accessModes that are sufficient to understand all the intellectual content of a resource. Expected values include:  auditory, tactile, textual, visual.
	AccessModeSufficient string `json:"accessModeSufficient,omitempty"` // types : Text
	...
	AccessibilityAPI string `json:"accessibilityAPI,omitempty"` // types : Text
	...
	AccessibilityControl string `json:"accessibilityControl,omitempty"` // types : Text
	...
	AccessibilityFeature string `json:"accessibilityFeature,omitempty"` // types : Text
	...
	AccessibilityHazard string `json:"accessibilityHazard,omitempty"` // types : Text
	...
	AccessibilitySummary string `json:"accessibilitySummary,omitempty"` // types : Text

	// AccountablePerson see : https://schema.org/accountablePerson
	// Specifies the Person that is legally accountable for the CreativeWork.
	AccountablePerson *Person `json:"accountablePerson,omitempty"` // types : Person
	...
	AlternativeHeadline string `json:"alternativeHeadline,omitempty"` // types : Text

	// AssociatedMedia see : https://schema.org/associatedMedia
	// A media object that encodes this CreativeWork. This property is a synonym for encoding.
	AssociatedMedia *MediaObject `json:"associatedMedia,omitempty"` // types : MediaObject
	...
	Audio *AudioObject `json:"audio,omitempty"` // types : AudioObject
	...
	Character *Person `json:"character,omitempty"` // types : Person

	// Citation see : https://schema.org/citation
	// A citation or reference to another creative work, such as another publication, web page, scholarly article, etc.
	Citation interface{} `json:"citation,omitempty"` // types : CreativeWork Text
	...
	CommentCount float64 `json:"commentCount,omitempty"` // types : Integer

	// ContentLocation see : https://schema.org/contentLocation
	// The location depicted or described in the content. For example, the location in a photograph or painting.
	ContentLocation *Place `json:"contentLocation,omitempty"` // types : Place

	// ContentRating see : https://schema.org/contentRating
	// Official rating of a piece of content—for example,'MPAA PG-13'.
	ContentRating string `json:"contentRating,omitempty"` // types : Text

	// ContentReferenceTime see : http://pending.google.schema.org/contentReferenceTime
	// The specific time described by a creative work, for works (e.g. articles, video objects etc.) that emphasise a particular moment within an Event.
	ContentReferenceTime DateTime `json:"contentReferenceTime,omitempty"` // types : DateTime

	// Contributor see : https://schema.org/contributor
	// A secondary contributor to the CreativeWork or Event.
	Contributor interface{} `json:"contributor,omitempty"` // types : Organization Person

	// CopyrightHolder see : https://schema.org/copyrightHolder
	// The party holding the legal copyright to the CreativeWork.
	CopyrightHolder interface{} `json:"copyrightHolder,omitempty"` // types : Organization Person

	// CopyrightYear see : https://schema.org/copyrightYear
	// The year during which the claimed copyright for the CreativeWork was first asserted.
	CopyrightYear float64 `json:"copyrightYear,omitempty"` // types : Number

	// Creator see : https://schema.org/creator
	// The creator/author of this CreativeWork. This is the same as the Author property for CreativeWork.
	Creator interface{} `json:"creator,omitempty"` // types : Organization Person
	...
	DatePublished Date `json:"datePublished,omitempty"` // types : Date

	// DiscussionUrl see : https://schema.org/discussionUrl
	// A link to the page containing the comments of the CreativeWork.
	DiscussionUrl string `json:"discussionUrl,omitempty"` // types : URL

	// Editor see : https://schema.org/editor
	// Specifies the Person who edited the CreativeWork.
	Editor *Person `json:"editor,omitempty"` // types : Person

	// EducationalAlignment see : https://schema.org/educationalAlignment
	// An alignment to an established educational framework.
	EducationalAlignment *AlignmentObject `json:"educationalAlignment,omitempty"` // types : AlignmentObject

	// EducationalUse see : https://schema.org/educationalUse
	// The purpose of a work in the context of education; for example, 'assignment', 'group work'.
	EducationalUse string `json:"educationalUse,omitempty"` // types : Text
	...
	Encoding *MediaObject `json:"encoding,omitempty"` // types : MediaObject
	...
	ExampleOfWork *CreativeWork `json:"exampleOfWork,omitempty"` // types : CreativeWork
	...
	Expires Date `json:"expires,omitempty"` // types : Date
	...
	FileFormat string `json:"fileFormat,omitempty"` // types : Text URL
	...
	HasPart *CreativeWork `json:"hasPart,omitempty"` // types : CreativeWork

	// Headline see : https://schema.org/headline
	// Headline of the article.
	Headline string `json:"headline,omitempty"` // types : Text
	...
	InteractionStatistic *InteractionCounter `json:"interactionStatistic,omitempty"` // types : InteractionCounter

	// InteractivityType see : https://schema.org/interactivityType
	// The predominant mode of learning supported by the learning resource. Acceptable values are 'active', 'expositive', or 'mixed'.
	InteractivityType string `json:"interactivityType,omitempty"` // types : Text
	...
	IsBasedOn interface{} `json:"isBasedOn,omitempty"` // types : CreativeWork Product URL

	// IsFamilyFriendly see : https://schema.org/isFamilyFriendly
	// Indicates whether this content is family friendly.
	IsFamilyFriendly bool `json:"isFamilyFriendly,omitempty"` // types : Boolean
	...
	IsPartOf *CreativeWork `json:"isPartOf,omitempty"` // types : CreativeWork

	// Keywords see : https://schema.org/keywords
	// Keywords or tags used to describe this content. Multiple entries in a keywords list are typically delimited by commas.
	Keywords string `json:"keywords,omitempty"` // types : Text

	// LearningResourceType see : https://schema.org/learningResourceType
	// The predominant type or kind characterizing the learning resource. For example, 'presentation', 'handout'.
	LearningResourceType string `json:"learningResourceType,omitempty"` // types : Text

	// License see : https://schema.org/license
	// A license document that applies to this content, typically indicated by URL.
	License interface{} `json:"license,omitempty"` // types : CreativeWork URL

	// LocationCreated see : https://schema.org/locationCreated
	// The location where the CreativeWork was created, which may not be the same as the location depicted in the CreativeWork.
	LocationCreated *Place `json:"locationCreated,omitempty"` // types : Place
	...
	MainEntity *Thing `json:"mainEntity,omitempty"` // types : Thing
	...
	Mentions *Thing `json:"mentions,omitempty"` // types : Thing
	...
	Producer interface{} `json:"producer,omitempty"` // types : Organization Person
	...
	Publication *PublicationEvent `json:"publication,omitempty"` // types : PublicationEvent

	// Publisher see : https://schema.org/publisher
	// The publisher of the creative work.
	Publisher interface{} `json:"publisher,omitempty"` // types : Organization Person

	// PublisherImprint see : http://bib.google.schema.org/publisherImprint
	// The publishing division which published the comic.
	PublisherImprint *Organization `json:"publisherImprint,omitempty"` // types : Organization
	...
	RecordedAt *Event `json:"recordedAt,omitempty"` // types : Event

	// ReleasedEvent see : https://schema.org/releasedEvent
	// The place and time the release was issued, expressed as a PublicationEvent.
	ReleasedEvent *PublicationEvent `json:"releasedEvent,omitempty"` // types : PublicationEvent
	...
	SchemaVersion string `json:"schemaVersion,omitempty"` // types : Text URL

	// SourceOrganization see : https://schema.org/sourceOrganization
	// The Organization on whose behalf the creator was working.
	SourceOrganization *Organization `json:"sourceOrganization,omitempty"` // types : Organization
	...
	SpatialCoverage *Place `json:"spatialCoverage,omitempty"` // types : Place
	...
	TemporalCoverage interface{} `json:"temporalCoverage,omitempty"` // types : DateTime Text URL

	// Text see : https://schema.org/text
	// The textual content of this CreativeWork.
	Text string `json:"text,omitempty"` // types : Text

	// ThumbnailUrl see : https://schema.org/thumbnailUrl
	// A thumbnail image relevant to the Thing.
	ThumbnailUrl string `json:"thumbnailUrl,omitempty"` // types : URL

	// TimeRequired see : https://schema.org/timeRequired
	// Approximate or typical time it takes to work with or through this learning resource for the typical intended target audience, e.g. 'P30M', 'P1H25M'.
	TimeRequired *Duration `json:"timeRequired,omitempty"` // types : Duration

	// TranslationOfWork see : http://bib.google.schema.org/translationOfWork
	// The work that this work has been translated from. e.g. 物种起源 is a translationOf “On the Origin of Species” Inverse property: workTranslation (see: https://schema.orghttp://bib.google.schema.org/workTranslation).
	TranslationOfWork *CreativeWork `json:"translationOfWork,omitempty"` // types : CreativeWork

	// Translator see : https://schema.org/translator
	// Organization or person who adapts a creative work to different languages, regional differences and technical requirements of a target market, or that translates during some event.
	Translator interface{} `json:"translator,omitempty"` // types : Organization Person
	...
	Version interface{} `json:"version,omitempty"` // types : Number Text

	// Video see : https://schema.org/video
	// An embedded video object.
	Video *VideoObject `json:"video,omitempty"` // types : VideoObject
	...
	WorkExample *CreativeWork `json:"workExample,omitempty"` // types : CreativeWork

	// WorkTranslation see : http://bib.google.schema.org/workTranslation
	// A work that is a translation of the content of this work. e.g. 西遊記 has an English workTranslation “Journey to the West”,a German workTranslation “Monkeys Pilgerfahrt” and a Vietnamese  translation Tây du ký bình khảo. Inverse property: translationOfWork (see: https://schema.orghttp://bib.google.schema.org/translationOfWork).
	WorkTranslation *CreativeWork `json:"workTranslation,omitempty"` // types : CreativeWork

}

func (v Sculpture) MarshalJSONWithTypeContext() ([]byte, error) {
	v.C = "http://schema.org"
	v.T = "Sculpture"
...
func (v *Sculpture) MarshalJSON() ([]byte, error) {
	if v == nil {
