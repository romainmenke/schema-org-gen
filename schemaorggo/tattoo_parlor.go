...
// TattooParlor see : https://schema.org/TattooParlor
type TattooParlor struct {
	typeContext

	// With properties from HealthAndBeautyBusiness see : https://schema.org/HealthAndBeautyBusiness
	//
...
func (v TattooParlor) intoMap(intop *map[string]interface{}) error {
	if intop == nil {
		return nil
	}
...
func (v TattooParlor) asMap() (map[string]interface{}, error) {
	...
	data["@type"] = "TattooParlor"

	return data, nil
}

func (v TattooParlor) MarshalJSON() ([]byte, error) {
	data, err := v.asMap()
	if err != nil {
		return nil, err
	}
